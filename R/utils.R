...
#' @return An list object of class `ccwarcs_options`
#' @export
...
validate_ccwarcs_options <- function(.options) {
  ...
  if (inherits(.options$cdx_sleep, c("numeric", "integer"))) {
    if (is.na(.options$cdx_sleep) | is.null(.options$cdx_sleep) | .options$cdx_sleep <= 0) {
  ...
  if (!is.null(.options$page_size)) {
    if (is.na(.options$page_size) ||
      ...
      .options$page_size < 1) {
      ArgumentCheck::addError("If provided, page_size must be a positive integer", ac)
    }
  }
  ...
  if (!dir.exists(path.expand(.options$cache))) {
    create_missing_dir <- utils::askYesNo(stringr::str_glue(
      "The folder {.options$cache} does not exist. Would you like to create it now?"
    ), default = TRUE)
